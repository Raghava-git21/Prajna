    ...
    mutationFn: async () => {
      ...
      Object.entries(form).forEach(([key, value]) => {
        if (['darshanSlots', 'aartiTimings', 'festivals', 'facilities'].includes(key)) {
          data.append(key, JSON.stringify(fromLines(value || '')));
        } else {
          data.append(key, value ?? '');
        }
      });
      if (isEdit && temple?.images) data.append('existingImages', JSON.stringify(temple.images));
      images.forEach((file) => data.append('images', file));
      return isEdit ? updateTemple(id, data) : createTemple(data);
    },
    onSuccess: () => {
    ...
    <form onSubmit={(e) => { e.preventDefault(); mutation.mutate(); }} className="space-y-6">
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="text-gray-600">Fill the temple profile, visitor details, and media.</p>
        </div>
