...
const AdminDashboard = () => {
  const { data: stats, isLoading } = useQuery({ queryKey: ['admin', 'stats'], queryFn: async () => (await getTempleStats()).data.data });
  const { data: recent } = useQuery({ queryKey: ['admin', 'recent'], queryFn: async () => (await getTemples({ sort: 'createdAt', order: 'DESC', limit: 5 })).data });
    ...
    <div>
      <div className="mb-6 flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="text-gray-600">Temple directory overview and recent records.</p>
        </div>
        <Link to="/admin/temples/new" className="btn-primary inline-flex items-center justify-center gap-2"><Plus size={18} /> Add Temple</Link>
      </div>
        ...
        <div className="rounded-lg bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 space-y-2">
            {(recent?.data || []).map((temple) => (
              <Link key={temple.id} to={`/admin/temples/${temple.id}/edit`} className="block rounded border border-saffron-100 px-3 py-2 text-sm hover:bg-saffron-50">
